...
def make_parser():
    ...
    parser = argparse.ArgumentParser(description="hal trains decoders", formatter_class=Formatter)
    Initializer.add_arguments(parser, default_arch='decoder')
...
def mp_main(rank, args):
    ...
    if ddp:
        ...
        if 'xla' in args.device:
            ...
            print(rank, device)
            init_process_group(backend=args.backend, init_method='pjrt://')
        else:
    ...
    elif 'xla' in device_type:
        import torch_xla.amp
        ptdtype = torch.bfloat16
        ctx = torch_xla.amp.autocast(device=device, dtype=ptdtype)
    else:
        device_type = 'cpu'
    ...
    # optimizer
    optimizer = configure_optimizers(model, args, device_type=device_type, decay_lm_head=False)
...
def main():
    ...
    if 'xla' in args.device:
        ...
        xmp.spawn(mp_main, args=(args,))
    else:
        mp_main(int(os.environ.get('RANK', -1)), args)
